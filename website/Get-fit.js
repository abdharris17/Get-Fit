...
function websiteSetup(){
    myBLE = new p5ble(); //Initialising Bluetooth

    const myCanvas = createCanvas(400, 200); //Creating canvas for the pedometer gauge
    myCanvas.parent('progress');

    //Initialising Settings Atrributes
    targetSteps = document.getElementById('step-goal');
    gender = document.getElementById('gender');
    height = document.getElementById('height');
    weight = document.getElementById('weight');

    //If stored data is available then  load the data
    ...
    //Initialising the pie chart in Activity Summary
    pieChartInit();

    //To Open the Settings page by default when website loads
